...
export const shuffleItems = async (seed) => {
  const huntMasterItem = OFFICE_ITEMS.find((item) => item.type === "person");
  const regularItems = OFFICE_ITEMS.filter((item) => item.type !== "person");

  const shuffledRegular = [...regularItems].sort(() => 0.5 - Math.random());
  const selectedRegular = shuffledRegular.slice(0, 4);

  if (!huntMasterItem) {
    return selectedRegular;
  }

  return [...selectedRegular, huntMasterItem];
};

export const getColleagueItems = async () => {
  try {
    const response = await fetch("/api/colleagues");
    if (!response.ok) return [];
    const data = await response.json();
    return (data.colleagues || []).map((colleague) => ({
      id: `colleague-${colleague._id}`,
      name: colleague.name,
      type: "colleague",
      colleagueId: colleague._id,
      riddle: `Find ${colleague.name}!`,
      faceDescriptor: colleague.faceDescriptor,
    }));
  } catch (error) {
    console.error("Failed to fetch colleague items:", error);
    return [];
  }
};
