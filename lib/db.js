...
export async function connectDB() {
  try {
    ...
    if (!mongoUri) {
      const errorMsg =
        "MONGODB_URI environment variable is not set. Add it to your local .env/.env.local (and Vercel env settings for production).";
    ...
    if (mongoUri.includes("<db_password>") || mongoUri.includes("<password>")) {
      const errorMsg =
        "MONGODB_URI still contains a placeholder password. Replace <db_password> with your real MongoDB Atlas password (URL-encode special characters).";
      console.error("❌ " + errorMsg);
      throw new Error(errorMsg);
    }
...
export async function saveColleague(name, riddle, faceDescriptor) {
  await connectDB();
  const colleague = await Colleague.findOneAndUpdate(
    { name },
    { name, riddle, faceDescriptor: Array.from(faceDescriptor) },
